...
`.agents/skill-configs/` 규칙은 에이전트 중립적이라 모든 스킬이 어떤 코딩 에이전트(Claude, Codex, ...)에서도 동일하게 동작합니다. 설정이 레거시 경로에서만 발견되면 스킬은 그것을 사용하면서 새 위치로 옮길지 제안합니다. spex는 이전 경로인 `.agent-workspace/spex/`로도 추가로 폴백합니다.
