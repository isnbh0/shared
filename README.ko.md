...
아래 호출 예시는 Claude Code 마켓플레이스 기준입니다. 다른 호스트에서는 해당 도구가 문서화한 스킬 활성화 방식을 사용하세요.
...
- 모델이 자발적으로 호출하지 않음 — `/gimme` 명시 호출 시에만 실행
...
아래 호출 예시는 Claude Code 스타일입니다. 다른 에이전트에서는 스킬 이름이나 설명으로 해당 워크플로우를 요청하세요.

#### run-optimize

graded task set을 기준으로 기존 skill 또는 prompt를 최적화하는 대화형 SkillOpt 오케스트레이션 도구입니다.

```
/run-optimize [path-to-seed-skill]
```

- train/val/test graded task 준비, backend 선택, 실행 승인, 결과 해석, write-back을 에이전트가 단계적으로 진행
- 기본값은 SkillOpt Guard를 통한 Codex이며 Claude와 pi/GLM 변형도 지원
- 먼저 [SkillOpt 프롬프트 가이드](tooling/skillopt/README.ko.md)를 보세요. 자세한 backend 설정은 [tooling/skillopt/SETUP.ko.md](tooling/skillopt/SETUP.ko.md)에 있습니다.
...
파일을 생성하는 스킬(interview, spex, report-writer, macros, study, gimme, run-optimize)은 계층적 우선순위를 가진 워크스페이스 디렉터리 설정을 지원합니다 (먼저 발견된 항목 우선):
...
4. **레거시 폴백** (`.claude/skill-configs/<skill>/`) — 오래된 설치

대부분의 파일 생성 스킬에는 기본값이 없습니다. 각 스킬은 첫 사용 시 설정을 안내합니다. 출력은 `.agent-workspace/<folder>` 규칙을 따릅니다 (`specs`, `reports`, `interviews`, `macros`, `study`, `gimme`). `run-optimize`는 기본적으로 `tooling/skillopt/runs` 아래에 SkillOpt 실행 결과를 씁니다.

새 설정은 `.agents/skill-configs/`를 사용하세요. `.claude/skill-configs/` 경로는 오래된 설치를 위한 레거시 폴백으로만 유지됩니다.
