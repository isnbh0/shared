...
서브에이전트 오케스트레이션 워크플로우 및 세션 모드: map-reduce, 리서치 기반 코드 비평, 합의 리뷰, 순차 패스, 리고 모드.
...
- **dredge** — `~/.claude/projects` 아래의 이전 Claude Code 대화 기록에서 문맥을 검색; 기본은 현재 프로젝트, 쿼리의 자연어 힌트(예: "모든 프로젝트에서", "craken 저장소에서", "어제")에 따라 범위·시간 창을 자동 확장
