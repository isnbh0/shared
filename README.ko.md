...
- **dredge** — `~/.claude/projects` 아래의 이전 Claude Code 대화 기록에서 문맥을 검색; 기본은 현재 프로젝트, 쿼리의 자연어 힌트(예: "모든 프로젝트에서", "craken 저장소에서", "어제")에 따라 범위·시간 창을 자동 확장. `agentsview` CLI가 설치되어 있으면 선택적 AgentsView 백엔드를 사용(사용자 스코프 `~/.claude/skill-configs/dredge/`에서 설정); 없으면 grep으로 폴백
