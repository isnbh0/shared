...
`SKILL.md` 스킬을 직접 읽는 에이전트에서 사용합니다.

```bash
# 원하는 위치에 클론
git clone <repo-url> ~/shared

# Codex, Amp, Antigravity 등 .agents/skills를 읽는 에이전트
...
Gemini CLI는 보통 설치된 `SKILL.md`를 명시적으로 읽는 extension, command, context 파일 구성이 필요합니다. 자세한 연결 방식은 크로스 플랫폼 가이드를 참고하세요.
...
아래 호출 예시는 Claude Code 마켓플레이스 기준입니다. 다른 에이전트는 보통 자연어 매칭, 명시적 스킬 언급, 자체 명령 시스템으로 스킬을 활성화합니다.
...
파일을 생성하는 스킬(interview, spex, report-writer, macros, study, gimme)은 계층적 우선순위를 가진 워크스페이스 디렉터리 설정을 지원합니다 (먼저 발견된 항목 우선):
...
기본값은 없습니다. 각 스킬은 첫 사용 시 설정을 안내합니다. 출력은 `.agent-workspace/<folder>` 규칙을 따릅니다 (`specs`, `reports`, `interviews`, `macros`, `study`, `gimme`).

새 설정은 `.agents/skill-configs/`를 사용하세요. `.claude/skill-configs/` 경로는 오래된 설치를 위한 레거시 폴백으로만 유지됩니다.

## 다른 에이전트 도구

이 스킬들은 여러 AI 코딩 도구에서 지원되는 Agent Skills `SKILL.md` 형식을 사용합니다. 설치 방법은 [크로스 플랫폼 가이드](docs/cross-platform/README.md)를 참조하세요:
