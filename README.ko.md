...
map-reduce 패턴을 활용한 병렬 작업 오케스트레이션 도구입니다.

```
/macros__mapreduce <task> [--workspace <dir>]
```

- 작업을 독립적인 청크로 분할하여 병렬 서브에이전트에 배포
- map 단계 전 선택적 공유 컨텍스트 구축
- 최종 reducer 서브에이전트가 모든 청크 리포트를 단일 결과물로 통합
...
파일을 생성하는 스킬(interview, spex, report-writer, macros)은 계층적 우선순위를 가진 워크스페이스 디렉터리 설정을 지원합니다 (먼저 발견된 항목 우선):
