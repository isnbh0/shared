...
아래에서는 스킬을 일반 이름으로 표시합니다. 활성화 문법은 호스트와 설치 방식에 따라 다르므로 [크로스 플랫폼 가이드](docs/cross-platform/README.md)를 참고하세요.
...
- **write-phased-gradual** — 다음의 유용한 증분 하나만 계획하는 점진적 다단계 명세 작성
- **implement** — 기존 명세를 따라 구현하고 상태를 업데이트한 뒤 커밋

#### macros

서브에이전트 오케스트레이션 워크플로우 및 동작 수정자: map-reduce, 순서 있는 분할 실행, 영속적 파일시스템 패킷, 점진적 정신 모델 캘리브레이션, 해석상의 재량, 리서치 기반 코드 비평, 합의 리뷰, 순차 패스, 리고 모드.

- **mapreduce** — 작업을 독립적인 청크로 분할, 병렬 서브에이전트 배포, 결과 통합
- **chunked** — 작업을 전체를 덮는 순서 있는 부분들로 분할 실행; 각 반복은 이전 반복의 산출물을 읽을 수 있음
- **packet** — 유연한 구조로 작업을 영속적인 파일시스템 핸드오프로 패키징하고, 반환에 의존하는 작업만 일시 중지한 뒤 반환 계약의 실질적 충족 여부를 검증하고 재개
- **calibrate** — 넓은 구조에서 결과에 영향을 주는 세부 구분으로 적응형 패스를 거쳐 정신 모델이나 상태를 정렬; packet과 조합하여 파일 기반 교환 가능
- **leeway** — 사용자가 의도한 범위에서 해석상의 재량을 부여해 불필요한 경직성·문자적 해석을 피하되, 새 프로토콜로 만들지는 않음
...
- **rigor** — 활성 중에는 정확성, 철저한 조사, 웹 기반 검증을 최소주의보다 우선시
- **orchestrate** — 활성 중에는 실행을 기본적으로 서브에이전트에 위임하고 높은 수준에서 작업하며, 자신의 컨텍스트는 방향 설정과 종합에 사용; 사소한 작업은 인라인으로 처리하는 예외 장치 포함
...
- **dry-run** — 일회성 안전장치: 활성한 요청을 실행하지 않고 무엇을 할지 설명한 뒤 확인을 기다림
...
- **new** — 커스텀 매크로 스캐폴딩: 일급 매크로처럼 동작하며 다른 활성 스킬과 조합 가능한 사용자 또는 프로젝트 스코프 스킬을 생성; 사용자 스코프 매크로는 기본적으로 `my-` 이름 접두어 사용
...
- 시크릿은 번들에 들어가지 않음 — API 키·토큰은 OS 시크릿 저장소 저장 명령으로 안내하고, 번들에는 참조 이름만 기록
- 선택 설정 `launch_command` (예: `cursor {path}`, `code {path}`, `open {path}`)로 번들을 에디터에서 즉시 오픈
- 모델이 자발적으로 호출하지 않음 — gimme 스킬을 명시적으로 활성화할 때만 실행
...
직접 설치한 스킬은 프론트매터의 `name`을 사용하며, 이 저장소에서는 소스 디렉터리 이름과 같습니다.

#### phaser

Phaser 3 게임 개발을 위한 검증된 패턴과 모범 사례 모음입니다.

Phaser 작업 시 의미 기반으로 활성화되는 패시브 지식 베이스입니다.
