...
the diagram names the decision and the shape of the answer. Stack both under the label,
separated by Mermaid's `<br>`. Where the substrate is already chosen, use its own
vocabulary.
...
Draw the uncertain branch whenever the machine acts on its judgments.
