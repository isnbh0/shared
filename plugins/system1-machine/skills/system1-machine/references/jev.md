...
questions and returns typed answers with probabilities, rather than generated text.

This page covers only the properties that change a *design*. The API contract — request
and response fields, SDK usage, current limits — lives in the live docs at
...
Worth picking while designing, because the shape determines what code can do with the
answer:

- **Choice** — which of these options. Use when the outcomes are a known closed set.
- **Score** — which level, against ordered descriptive levels. Use for graded judgments
  you intend to threshold or weight.
...
answer, that is a deliberate second request, not a free composition.
...
when not — needs Choice or Score at that decision point. A design that wants Noul there
can derive its own measure from the probability, or move the uncertainty gate elsewhere.
