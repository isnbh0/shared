...
1. If another skill is active in this invocation (e.g., `/chunked /mapreduce ...`), that skill defines the per-chunk job and prompt template — chunked's role is to slice the scope it operates on.
2. Otherwise, use the most recent substantive task or request in the conversation.
3. If no task can be determined, STOP and tell the user: "No task found. Provide a task description or compose with another skill (e.g., `/chunked /mapreduce <task>`)."
...
- **Don't parallelize across chunks.** Sequential ordering is the whole point — parallelism belongs *inside* a chunk (via composed `/macros:mapreduce`), not across them.
