...
def shift_tab_to(page, selector):
    for _ in range(10):
...
def test_crumb_navigation(zpage):
    page = zpage
    page.keyboard.press(IN)
    page.keyboard.press("ArrowDown")
    page.keyboard.press(IN)
    assert zoom(page) == "1.1"
    assert crumbs(page) == ["Zoom", "1 One"]
    shift_tab_to(page, "#crumbs li:first-child a")
...
def test_crumb_focus_ring(open_outline):
    page = open_outline(ZOOM_MD, "#z=1.1")
    shift_tab_to(page, "#crumbs li:last-child a")
    style = page.evaluate(
