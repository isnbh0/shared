...
@pytest.fixture
def example_page(tmp_path):
    markdown = (SKILL_DIR / "examples/tractatus.md").read_text(encoding="utf-8")
    page = tmp_path / "tractatus.html"
    page.write_text(embed.embed(markdown, template_text()), encoding="utf-8")
    return page
...
def test_targets_and_focus(panel, browser_name):
    page = panel
    sizes = page.evaluate(
...
def test_persists_across_reload(panel):
    ...
    assert abs(float(style(page, "#item-1 > .row", "lineHeight").removesuffix("px")) - 17 * 1.65) < 0.01
