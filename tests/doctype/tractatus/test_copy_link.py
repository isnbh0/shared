...
def test_fallback(links, browser_name):
    page = links()
    if browser_name == "webkit":
        # Headless WebKit declines execCommand after the async clipboard rejection.
        # Simulate its copy event so this still checks the viewer's fallback path.
        page.evaluate("""() => { document.execCommand = () => {
          const values = new Map();
          const event = new Event('copy', { bubbles: true, cancelable: true });
          Object.defineProperty(event, 'clipboardData', { value: {
            setData: (type, text) => values.set(type, text),
